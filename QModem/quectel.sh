...
sim_info()
{
    ...
    if [ "$isp" = "CHN-CMCC" ] || [ "$isp" = "CMCC" ] || [ "$isp" = "46000" ] || [ "$isp" = "46002" ] || [ "$isp" = "46004" ] || [ "$isp" = "46007" ] || [ "$isp" = "46008" ]; then
        isp="中国移动"
    elif [ "$isp" = "CHN-UNICOM" ] || [ "$isp" = "UNICOM" ] || [ "$isp" = "CUCC" ] || [ "$isp" = "46001" ] || [ "$isp" = "46006" ]; then
        isp="中国联通"
    elif [ "$isp" = "CHN-CT" ] || [ "$isp" = "CHN-TELECOM" ] || [ "$isp" = "CT" ] || [ "$isp" = "CTCC" ] || [ "$isp" = "46003" ] || [ "$isp" = "46011" ]; then
        isp="中国电信"
    elif [ "$isp" = "CBN" ] || [ "$isp" = "46015" ]; then
        isp="中国广电"
    fi
    ...
    #ICCID（集成电路卡识别码）
    at_command="AT+CCID"
	iccid=$(at $at_port $at_command | grep -o "+CCID:[ ]*[-0-9A-F]\+" | cut -d " " -f 2 )

    # 获取CCID原始数据作为备选方案
    if [ -z "$iccid" ]; then
        at_command="AT+CCID"
        ccid_raw=$(at $at_port $at_command)
        iccid=$(echo "$ccid_raw" | grep -o '[0-9]\{19,20\}' | head -n 1)
    fi
...
get_lockband_lte()
{
    ...
    add_avalible_band_entry "40" "B40"
...
EOF
            ...
            "LTE"|"CAT-M"|"CAT-NB")
                ...
                lte_ul_bandwidth=$(get_bandwidth "LTE" $ul_width_num)
...
# ==============================================================================
# 🌟 新增：重写或扩展系统的 get_sim_info 流程，强行注入 ICCID 查询与汉化所需组件
# ==============================================================================

# 注意：由于 generic.sh 已经定义了标准的 get_sim_info
# 我们在这里写一个补充钩子，确保每次获取 SIM卡信息时，顺便带上 ICCID
get_sim_info_quectel_addon() {
    # 1. 确保串口可用
    [ -z "$at_port" ] && return

    # 2. 发送移远标准指令获取 CCID 原始数据
    local ccid_raw
    ccid_raw=$(at "$at_port" "AT+CCID")

    # 3. 提取出 19 位或 20 位的纯数字 ICCID
    local iccid_value
    iccid_value=$(echo "$ccid_raw" | grep -o '[0-9]\{19,20\}' | head -n 1)

    # 4. 如果成功拿到了数据，用系统的 jshn 库将其打包，格式对齐 overview.js
    if [ -n "$iccid_value" ]; then
        json_add_object
        json_add_string "type" "plain_text"
        json_add_string "class" "SIM Information"      # 分类划入"SIM卡信息"大卡片
        json_add_string "key" "iccid"            # 前端用于代码卡片高亮的识别主键
        json_add_string "full_name" "ICCID"      # 网页上显示的左侧标签名
        json_add_string "value" "$iccid_value"   # 实际数据
        json_close_object
    fi
}

# 强行劫持/包裹原有的 get_sim_info 函数，实现无缝注入
if [ "$(type -t get_sim_info)" = "function" ]; then
    eval "original_get_sim_info=\"$(declare -f get_sim_info | sed '1,2d;$d')\""
    get_sim_info() {
        # 先执行原系统 generic.sh 里的查询（查驻网、状态、IMSI等）
        eval "$original_get_sim_info"
        # 紧接着塞入我们定制的 ICCID 查询
        get_sim_info_quectel_addon
    }
fi
